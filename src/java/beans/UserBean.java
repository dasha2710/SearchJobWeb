...
public class UserBean implements Serializable {
    ...
    private User user;
    @Pattern(regexp = "^[A-Za-z0-9_-]{6,18}$", message = "{invalidPassword}")
    ...
    public void sendPassword() {
        ...
        if (temp == null) {
            ...
            String pass = util.PasswordGenerator.generate();
            user.setPassword(util.Encrypt.encryptSHA256(pass));
            if (sendingEmail.SendingEmail.sendEmail(user.getEmail(), "New password is " + pass)) {
                userDAO.update(user);
                ResourceBundle bundle = ResourceBundle.getBundle("resources.jsf.texts", FacesContext.getCurrentInstance().getViewRoot().getLocale());
    ...
    public String changePassword(){
        oldPassword = util.Encrypt.encryptSHA256(oldPassword);
        ...
        newPassword = util.Encrypt.encryptSHA256(newPassword);
    ...
    public String register() {
        try {
            user.setPassword(util.Encrypt.encryptSHA256(passwordConfirmation));
            userDAO.create(user);
        } catch (Exception e) {
            ResourceBundle bundle = ResourceBundle.getBundle("resources.jsf.texts", FacesContext.getCurrentInstance().getViewRoot().getLocale());
        ...
        try {
            ...
            getRequest().login(user.getEmail(), passwordConfirmation);
        } catch (ServletException ex) {
            Logger.getLogger(UserBean.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public String login() {
        try {
            ...
            user.setPassword(util.Encrypt.encryptSHA256(user.getPassword()));
            User userTemp = userDAO.findUser(user.getEmail(), user.getPassword());
            if (userTemp == null) {
                ResourceBundle bundle = ResourceBundle.getBundle("resources.jsf.texts", FacesContext.getCurrentInstance().getViewRoot().getLocale());
    ...
    public void changeStatusVacancy() {
        try {
